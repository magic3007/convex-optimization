...
\end{aligned}
$$
其中$f(x)=\mu\lVert x \rVert_{1,2}$, $g(y)=\frac{1}{2}\lVert y \rVert_F^2$, 其拉格朗日函数为
$$
...
设$p(x)=\lVert x \rVert_2(x \in R^l)$为向量的2范数, 其对偶范数为其本身, 故其共轭函数为$p^*(x)=\left \{ \begin{aligned} 0,&\ \lVert x \rVert_2 \leq 1 \\ \infty,&\ otherwise \end{aligned} \right.$. 由于$f(x)=\mu\sum\limits_{i=1}^{n}p(x_i)$, 故$f^*(x)=\sum\limits_{i=1}^{n}p^*(\frac{1}{\mu}x_i)$. 另一方面, 容易得到$g$的共轭函数即为自身, 即$g^*(z)=g(z)$. 故其对偶问题为:

$$
\begin{aligned}
\min_{z,u} &\  g(z)+\langle b,z\rangle \\
\text{s.t.} &\ u+A^Tz =0 \\
&\lVert u_i \rVert_2 \leq \mu,\ i=1,\ldots, n
\end{aligned}
$$
...
$$
\begin{align}
L_\rho(z,u,x)&=g(z)+\langle b,z \rangle - \langle x, u+A^Tz \rangle + \frac{\rho}{2}\lVert u+A^Tz\rVert_F^2\\
\text{s.t.}\ \lVert u_i \rVert_2 &\leq \mu,\ i=1,\ldots, n\\
\end{align}
$$
迭代方式为:
$$
...
\end{aligned}
$$
其中$f(x)=\mu\lVert x \rVert_{1,2}$, $g(y)=\frac{1}{2}\lVert Ay-b \rVert_F^2$.

对于原问题的增广拉格朗日函数为:
$$
L_t(x,y,z) = f(x) + g(y) - \langle z, x-y \rangle + \frac{\rho}{2}\lVert x-y \rVert_F^2
$$
使用交替方向乘子法, 其迭代更新方式为:
$$
\begin{align}
y^{k+1} &= arg\min_y L_{\rho}(x^{k},y, z^k) = (\rho I+A^TA)^{-1}(A^Tb-z^k+\rho x^{k}) \\
x^{k+1} &= arg\min_x L_{\rho}(x,y^{k+1}, z^k) \\ 
 	   &= arg \min_x \mu\lVert x \rVert_{1,2} - \langle z^k, x \rangle + \frac{\rho}{2}\lVert x-y^{k+1} \rVert_F^2 \\
		&= arg \min_x \mu\lVert x \rVert_{1,2} + \frac{\rho}{2}\lVert x - y^{k+1} - \frac{1}{\rho}z^k\rVert_F^2 \\
		&= prox_{\frac{1}{\rho}f}(y^{k+1}+\frac{1}{\rho}z^k)\\
...
$$
\begin{align}
x^{k+1} &= arg \min_x \mu \lVert x \rVert_{1,2} + \rho \langle x^{k} -y^{k+1}-\frac{1}{\rho}z^k, x\rangle + \frac{1}{2\eta_k}\lVert x - x^k \rVert_F^2 \\
		&= arg \min_x \mu \lVert x \rVert_{1,2} + \frac{1}{2\eta_k}\lVert x - x^k + \eta_k\rho(x^{k} -y^{k+1}-\frac{1}{\rho}z^k)\rVert_F^2 \\
		&= arg \min_x prox_{\eta_k f}(x_k - \eta_k\rho(x^{k} -y^{k+1}-\frac{1}{\rho}z^k))
\end{align}
$$
